...
class RWMB_Group_Field extends RWMB_Field {
	...
	public static function html( $meta, $field ) {
		...
		foreach ( $field['fields'] as $child_field ) {
			if ( isset( $child_field['file_input_name'] ) ) {
				$child_field['file_input_name'] = self::child_field_name( $field['field_name'], $child_field['file_input_name'] );
				$child_field['file_input_name'] = self::convert_file_name( $child_field['file_input_name'] );
			}
	...
	protected static function convert_file_name( $name ) {
		$sep = '@@@';
	...
	public static function value( $new, $old, $post_id, $field ) {
		...
		if ( ! isset( $field['group_name'] ) ) {
			$field['group_name'] = $field['id'];
		}

		foreach ( $child_fields as $child_field ) {

			if ( 'group' === $child_field['type'] ) {
				$child_field['group_name'] = $field['group_name'] . '@@@' . $child_field['id'];
				$new_group = isset( $new[ $child_field['id'] ] ) ? $new[ $child_field['id'] ] : array();
				$old_group = isset( $old[ $child_field['id'] ] ) ? $old[ $child_field['id'] ] : array();

				$new[ $child_field['id'] ] = self::value( $new_group, $old_group, $post_id, $child_field );
			}
			...
			$child_field['group_name'] = $field['group_name'] . '@@@' . $child_field['file_input_name'];
			$old_upload = ! empty( $new[ $child_field['id'] ] ) ? $new[ $child_field['id'] ] : array();
			$value = self::value_file( array(), $old_upload, $post_id, $child_field, $field );
	...
	/**
	 * Get meta values to save for file field.
	 *
	 * @param mixed $new          The submitted meta value.
	 * @param mixed $old          The existing meta value.
	 * @param int   $post_id      The post ID.
	 * @param array $field        The field parameters.
	 * @param array $parent_field The parent field parameters.
	 *
	 * @return array|mixed
	 */
	protected static function value_file( $new, $old, $post_id, $field, $parent_field ) {
		$file_input_name = $field['group_name'];

		$count = self::transform_file( $file_input_name, $parent_field['id'] );
	 ...
	 *
	 * @param string $group_id The field group ID.
	 * @param string $field_id The field ID.
	 *
	 * @return int The number of uploaded files.
	 */
	protected static function transform_file( $field_id, $group_id ) {
