<?php
/**
 * Additional features to customize menus
 *
 * @package PenShop
 */

/**
 * Class PenShop_Walker_Mega_Menu
 *
 * Walker class for mega menu
 */
class PenShop_Walker_Mega_Menu extends Walker_Nav_Menu {
	/**
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$item_mega = penshop_parse_args( $item_mega, penshop_get_mega_menu_setting_default() );
...
/**
 * Add icon in front of menu item title
 *
 * @param $title
 * @param $item
 *
 * @return string
 */
function penshop_menu_item_icon( $title, $item ) {
	$item_mega = get_post_meta( $item->ID, '_menu_item_mega', true );
	$item_mega = penshop_parse_args( $item_mega, penshop_get_mega_menu_setting_default() );

	if ( $item_mega['icon'] ) {
		$title = '<i class="' . esc_attr( $item_mega['icon'] ) . '"></i>' . $title;
	}

	return $title;
}

add_filter( 'nav_menu_item_title', 'penshop_menu_item_icon', 10, 2 );