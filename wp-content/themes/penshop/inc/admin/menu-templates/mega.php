<div id="smm-panel-mega" class="smm-panel-mega smm-panel">
	<p class="mega-settings">
		<span class="setting-field">
			<label>
				<?php esc_html_e( 'Enable mega menu', 'penshop' ) ?><br>
				<select name="{{ smm.getFieldName( 'mega', data.data['menu-item-db-id'] ) }}">
					<option value="0"><?php esc_html_e( 'No', 'penshop' ) ?></option>
					<option value="1" {{ parseInt( data.megaData.mega ) ? 'selected="selected"' : '' }}><?php esc_html_e( 'Yes', 'penshop' ) ?></option>
				</select>
			</label>
		</span>
		...
		<span class="setting-field">
			<label>
				<?php esc_html_e( 'Mega panel width', 'penshop' ) ?><br>
